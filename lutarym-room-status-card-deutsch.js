...
class LutarymRoomStatusCard extends HTMLElement {
  ...
  _setRoomStatus(room, value) {
    ...
    const color = this._getRoomColorForStatus(room, value);
    ...
    // Set color attribute for history visualization
  ...
  // Get the color for a given status and room
  _getRoomColorForStatus(room, status) {
    if (!room) return '#000000';
    return room.status_colors?.[status] ?? STATUS_COLORS[status]?.bg ?? '#000000';
  }
